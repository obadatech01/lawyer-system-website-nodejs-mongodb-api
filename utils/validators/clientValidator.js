...
exports.createClientValidator = [
  check("name").notEmpty().withMessage("يرجى إدخال اسم العميل"),

  check("identificationNumber")
    .notEmpty()
    .withMessage("يرجى إدخال رقم هوية العميل")
    .custom((val) =>
    Client.findOne({ identificationNumber: val }).then((client) => {
        if (client) {
          return Promise.reject(
            new Error("رقم الهوية مكرر")
          );
        }
      })
    ),

  body("nationality").notEmpty().withMessage("يرجى إدخال جنسية العميل"),

  // body("gender")
  //   .notEmpty()
  //   .withMessage("يرجى إدخال جنس العميل"),

  body("phone").notEmpty().withMessage("يرجى إدخال رقم جوال العميل"),

  body("address").notEmpty().withMessage("يرجى إدخال عنوان العميل"),
...
exports.getClientValidator = [
  check("id")
    .isMongoId()
    .withMessage("تنسيق معرف العميل غير صالح!")
    .custom((val) =>
